// Hallmark · component: task row v7 · genre: utilitarian · theme: DMS dynamic
...
Rectangle {
    ...
    readonly property bool isInProgress: !isCompleted && Boolean(taskData.inProgress)
    ...
    readonly property bool showRecurrence: !isTrash && !isCompleted && Boolean(taskData.recurrence)
    ...
    Row {
        ...
        Item {
            ...
            MouseArea {
                ...
                ToolTip.delay: 800
                ToolTip.text: row.isCompleted ? "Reset task" : (row.isInProgress ? "Left-click to complete · Right-click to cancel" : "Start task")
                onClicked: mouse => {
                    if (mouse.button === Qt.RightButton) {
                        if (row.isInProgress)
                            row.pluginRoot.cancelTodoProgress(row.taskData.id)
                        return
                    }
                    row.pluginRoot.toggleTodo(row.taskData.id)
                }
            }
        }
        ...
        Item {
            ...
            StyledText {
                ...
                anchors.verticalCenter: parent.verticalCenter
                visible: row.showPriority || row.showDate || row.showRecurrence
                text: {
                    const parts = []
                    if (row.showPriority)
                        parts.push(row.pluginRoot.priorityLabel(row.taskData.priority))
                    if (row.showDate)
                        parts.push(row.pluginRoot.dueLabel(row.taskData.dueDate, row.taskData.dueTime))
                    if (row.showRecurrence)
                        parts.push("↻")
                    return parts.join(" · ")
                }
    ...
    Menu {
        ...
        MenuItem {
            ...
            height: visible ? 40 : 0
            visible: !row.isTrash && !row.isCompleted && row.pluginRoot.canSnooze(row.taskData)
            contentItem: StyledText {
                text: parent.text
                leftPadding: Theme.spacingS
                verticalAlignment: Text.AlignVCenter
            ...
            onTriggered: row.pluginRoot.snoozeTodo(row.taskData.id, 10)
        }
